    ...
    public class LemmaExample : IComparable<LemmaExample>, IComparer<LemmaExample> {
        ...
        public LemmaExample(string sWord, string sLemma, double dWeight, string sMsd, RuleList rlRules, LemmatizerSettings lsett) {
            ...
            if (lsett != null) {
                ...
                sSignature = "[" + sWord + "]==>[" + sLemma + "](" + (sMsd != null ? sMsd + "," : "") + dWeight + ")";
            }
        }
        ...
        public int Similarity(LemmaExample le) {
            return EqualPrifixLen(this.sWord, le.sWord, false);
        ...
        public static int EqualPrifixLen(string sStr1, string sStr2, bool bForward) {
            int iLen1 = sStr1.Length;
            int iLen2 = sStr2.Length;
            int iMinLen = Math.Min(iLen1, iLen2);

            if (bForward) {
                for (int iPos = 0; iPos < iMinLen; iPos++)
                    if (sStr1[iPos] != sStr2[iPos]) return iPos;
            }
            else {
                for (int iPos = 1; iPos <= iMinLen; iPos++)
                    if (sStr1[iLen1 - iPos] != sStr2[iLen2 - iPos]) return iPos-1;
            }
